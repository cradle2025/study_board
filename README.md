...
> 切换前确实生成了备份文件。
>
> `smoke:sync` 让主进程**直接往磁盘上写文件**来模拟 Obsidian / VS Code 保存，
> 来回七趟：外部新增要自己长出来、外部改动要刷新编辑器、外部删除要收起列表
> **并把指着它的卡片关联一起解开**、外部改名后卡片还得指着同一篇；
> 另外还验两件事——自己写盘**不能**触发事件回环（否则每打一个字编辑器就会
> 被重载一次），以及编辑器里有未保存内容时撞上外部改动**必须先问一句**。
